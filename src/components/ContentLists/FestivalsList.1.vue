...
export default {
	...
	methods: {
		getCurrentPage() {
			...
			this.$http.get(backendUrl + '/api/festivals/page?page=' + query.page + 
			...
			.then(response => {
				...
				this.festivals.forEach(festival => {
					festival.city = festival.address.city;
					festival.country = festival.address.country;
				});
